...
function Insights({ apiEndpoint }) {
  ...
  const countVehicles = (image) => {
    ...
    return image.edge_detected_object_detection.edge_detected_object_detection_results.filter(
      (result) => vehicleTypes.includes(result.name),
    ).length;
  };
  ...
  useEffect(() => {
    const fetchImages = async () => {
      try {
        ...
        const totalVehicles = reversedData.reduce(
          (sum, img) => sum + countVehicles(img),
          0,
        );
        setTotalVehicles(totalVehicles);

        setAverageSSIM(
          reversedData.reduce((sum, img) => sum + img.ssim, 0) /
            reversedData.length,
        );
        setAverageTimeToComplete(
          reversedData.reduce((sum, img) => sum + img.time_to_complete, 0) /
            reversedData.length,
        );
        setAverageF1Score(
          reversedData.reduce((sum, img) => sum + img.f1, 0) /
            reversedData.length,
        );
        const best = {
          ...
          hausdorff_distance: Math.min(
            ...reversedData.map((img) => img.hausdorff_distance),
          ),
          time_to_complete: Math.min(
            ...reversedData.map((img) => img.time_to_complete),
          ),
  ...
  const handleDelete = async (imageId) => {
    if (window.confirm("Are you sure you want to delete this image?")) {
      try {
        ...
        setImages(
          images.filter((img) => img.edge_detected_image_id !== imageId),
        );
      } catch (error) {
        setError(error.message);
      }
    ...
    <>
      ...
      <div className="flex flex-col justify-center items-center gap-8 w-full bg-[#1B1A46] py-4 px-8 mb-8 rounded border border-[#4d447a] text-md 1380px:text-lg lg:flex-row">
        ...
        <div className="flex flex-col items-center">
          ...
          <p className="text-white text-lg 1380px:text-2xl font-bold border-white border-2 px-2 py-1 rounded-md">
            {(averageTimeToComplete / 60_000).toFixed(2)} mins
          </p>
        </div>
        <div className="flex flex-col items-center">
          <p className="text-white font-bold mb-1">Fastest Processing Time</p>
          <p className="text-white text-lg 1380px:text-2xl font-bold border-white border-2 px-2 py-1 rounded-md">
            {(bestMetrics.time_to_complete / 60_000).toFixed(2)} mins
          </p>
        </div>
        <div className="flex flex-col items-center">
          <p className="text-white font-bold mb-1">Average SSIM Score</p>
          <p className="text-white text-lg 1380px:text-2xl font-bold border-white border-2 px-2 py-1 rounded-md">
            {averageSSIM.toFixed(2)}
          </p>
        </div>
        <div className="flex flex-col items-center">
          <p className="text-white font-bold mb-1">Max Qubits Used</p>
          <p className="text-white text-lg 1380px:text-2xl font-bold border-white border-2 px-2 py-1 rounded-md">
            {Math.log2(bestMetrics.tile_size * bestMetrics.tile_size) + 1}
          </p>
        </div>
        <div className="flex flex-col items-center">
          <p className="text-white font-bold mb-1">Average F1 Score</p>
          <p className="text-white text-lg 1380px:text-2xl font-bold border-white border-2 px-2 py-1 rounded-md">
            {averageF1Score.toFixed(2)}
          </p>
        </div>
      </div>
              ...
              <>
                <div className="flex flex-row gap-2 mt-4 mb-8">
                  <div className="hidden xl:flex flex-col gap-2 bg-[#1B1A46] py-4 px-8 rounded border border-[#4d447a] text-white text-md items-center font-bold">
                    ...
                    <p className="border-white border-2 px-2 py-1 rounded-md">
                      {image.tile_size * image.tile_size}
                    </p>
                    ...
                    <div className="flex items-center">
                      <p className="border-white border-2 px-2 py-1 rounded-md">
                        {(image.time_to_complete / 60_000).toFixed(2)} mins
                      </p>
                      {images.length > 1 &&
                        image.time_to_complete ===
                          bestMetrics.time_to_complete && (
                  ...
                  <div className="flex flex-col gap-2 bg-[#1B1A46] py-4 px-8 rounded border border-[#4d447a] w-full">
                    <label className="text-sm font-bold text-white mb-2 p-2 block tracking-widest w-fit whitespace-nowrap bg-[#34335A]">
                      Quantum Edge Detected Result:
                    </label>
                    <div className="flex flex-row items-center justify-center bg-[#39385E]">
                      <img
                        className="border-white border-2"
                        src={`data:image/jpeg;base64,${image.edge_detected_image_base64}`}
                        alt="Edge Detection"
                      />
                    </div>
                  </div>
                  <div className="hidden lg:flex flex-col gap-2 bg-[#1B1A46] py-4 px-8 rounded border border-[#4d447a] text-white text-md items-center font-bold">
                    ...
                    <div className="flex flex-col items-center gap-2">
                      ...
                      <div className="flex items-center">
                        <p className="border-white border-2 px-2 py-1 rounded-md">
                          {image.precision.toFixed(6)}{" "}
                        </p>
                        {images.length > 1 &&
                          image.precision === bestMetrics.precision && (
                            <span className="text-yellow-400 ml-2">
                              <FaStar size={18} />
                            </span>
                          )}
                    ...
                    <div className="flex flex-col items-center gap-2">
                      ...
                      <div className="flex items-center">
                        <p className="border-white border-2 px-2 py-1 rounded-md">
                          {image.recall.toFixed(6)}{" "}
                        </p>
                        {images.length > 1 &&
                          image.recall === bestMetrics.recall && (
                            <span className="text-yellow-400 ml-2">
                              <FaStar size={18} />
                            </span>
                          )}
                    ...
                    <div className="flex flex-col items-center gap-2">
                      ...
                      <div className="flex items-center">
                        <p className="border-white border-2 px-2 py-1 rounded-md">
                          {image.ssim.toFixed(6)}{" "}
                        </p>
                        {images.length > 1 &&
                          image.ssim === bestMetrics.ssim && (
                            <span className="text-yellow-400 ml-2">
                              <FaStar size={18} />
                            </span>
                          )}
                    ...
                    <div className="flex flex-col items-center gap-2">
                      ...
                      <div className="flex items-center">
                        ...
                        {images.length > 1 &&
                          image.hausdorff_distance ===
                            bestMetrics.hausdorff_distance && (
                            <span className="text-yellow-400 ml-2">
                              <FaStar size={18} />
                            </span>
                          )}
                ...
                <div className="flex flex-col gap-2 items-center justify-center">
                  <div className="flex gap-4">
                    <button
                      onClick={() =>
                        navigate(`/insights/${image.edge_detected_image_id}`)
                      }
